 ...
 */
class Tortuga_Pro_Magazine_Posts_List_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'tortuga-magazine-posts-list', // ID.
			sprintf( esc_html__( 'Magazine Posts: List (%s)', 'tortuga-pro' ), 'Tortuga Pro' ), // Name.
			array(
				'classname' => 'tortuga_magazine_posts_list',
				'description' => esc_html__( 'Displays your posts from a selected category in a simple list layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'tortuga-pro' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_tortuga_magazine_posts_list', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_tortuga_magazine_posts_list', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post(); ?>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix' ); ?>>
					...
					<a href="<?php esc_url( the_permalink() ); ?>" rel="bookmark">
						<?php the_post_thumbnail( 'tortuga-thumbnail-large' ); ?>
					</a>
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_tortuga_magazine_posts_list', 'widget' );
